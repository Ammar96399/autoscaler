...
function print_help {
  ...
  echo "The 'print' action will print all resources that would be used by, e.g., 'sudo k0s kubectl diff'."
...
for i in $COMPONENTS; do
  if [ $i == admission-controller-deployment ] ; then
    if [[ ${ACTION} == create || ${ACTION} == apply ]] ; then
      ...
      sudo k0s kubectl delete -f ${SCRIPT_ROOT}/deploy/admission-controller-service.yaml --ignore-not-found
    fi
  fi
  if [[ ${ACTION} == print ]]; then
    ...
    ${SCRIPT_ROOT}/hack/vpa-process-yaml.sh $(script_path $i) | sudo k0s kubectl ${ACTION} ${EXTRA_FLAGS} -f - || true
  fi
done
