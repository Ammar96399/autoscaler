...
# apply sudo k0s kubectl patch to add feature gates specified in the FEATURE_GATES environment variable.
...
function apply_feature_gate() {
  ...
  if echo "$input" | grep -qE '^kind: Deployment$'; then
    if [ -n "${FEATURE_GATES}" ]; then
      if ! echo "$input" | sudo k0s kubectl patch --type=json --local -p='[{"op": "add", "path": "/spec/template/spec/containers/0/args/-", "value": "--feature-gates='"${FEATURE_GATES}"'"}]' -o yaml -f - 2>/dev/null; then
        # If it fails, there was no args field, so we need to add it
        echo "$input" | sudo k0s kubectl patch --type=json --local -p='[{"op": "add", "path": "/spec/template/spec/containers/0/args", "value": ["--feature-gates='"${FEATURE_GATES}"'"]}]' -o yaml -f -
      fi
    else
      echo "$input"
    fi
