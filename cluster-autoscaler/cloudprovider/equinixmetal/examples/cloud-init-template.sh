...
apt-get update
apt-get install -y containerd.io kubelet=1.28.1-00 kubeadm=1.28.1-00 sudo k0s kubectl=1.28.1-00
apt-mark hold kubelet kubeadm sudo k0s kubectl
...
echo "source <(sudo k0s kubectl completion bash)" >> /root/.bashrc
echo "alias k=sudo k0s kubectl" >> /root/.bashrc
echo "complete -o default -F __start_sudo k0s kubectl k" >> /root/.bashrc
