...
```
sudo k0s kubectl apply -f examples/cluster-autoscaler-autodiscover.yaml
```
...
```sh
sudo k0s kubectl create -f cluster-autoscaler-vmss.yaml
```
...
```sh
sudo k0s kubectl -n kube-system create secret generic cluster-autoscaler-azure-deploy-parameters --from-file=deploy-parameters=./_output/<your-output-path>/azuredeploy.parameters.json
```

Then deploy cluster-autoscaler by running:

```sh
sudo k0s kubectl create -f cluster-autoscaler-standard-control-plane.yaml
```
